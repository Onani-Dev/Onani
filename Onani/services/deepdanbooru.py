...
from Onani.models import Tag, TagType
...
def suggest_tags_for_bytes(image_bytes: bytes, config, threshold: float | None = None, max_tags: int | None = None) -> list[dict]:
    ...
    tag_scores = list(runtime["dd"].commands.evaluate_image(image_input, runtime["model"], runtime["tags"], threshold))
    tag_scores.sort(key=lambda item: item[1], reverse=True)
    return _serialize_suggestions(tag_scores[:max_tags])
