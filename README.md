...
You'll be prompted for reCAPTCHA v2 keys. The script generates a `.env`
with `DB_PASSWORD`, `FLASK_SECRET_KEY`, and the keys you entered.
...
| `RECAPTCHA_PUBLIC_KEY`  | Yes      | reCAPTCHA site key         |
| `RECAPTCHA_PRIVATE_KEY` | Yes      | reCAPTCHA secret key       |
...
DATABASE_URL=sqlite:///:memory: FLASK_SECRET_KEY=test \
  RECAPTCHA_PUBLIC_KEY=test RECAPTCHA_PRIVATE_KEY=test \
